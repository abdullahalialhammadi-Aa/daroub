...
test('a scene lists catalogue species first, fills with terrain examples without repeating a drawing, then four essentials',()=>{
  const items=sceneItems('desert',[species('camel','Camel'),species('date-palm','Date palm','terrain-example')],200,6,null);
  ...
  assert.equal(sceneItems('desert',[],200,6,null).filter(i=>i.kind==='animal'||i.kind==='plant').length,5,'five desert examples without catalogue species');
  ...
  for(const focus of [null,'AE'])for(const terrain of ['desert','mountain','forest','coast']){const set=sceneItems(terrain,[],120,6,focus);assert.equal(set.length,9,terrain+' has five life examples and four essentials');assert.equal(new Set(set.map(i=>i.id)).size,9,terrain+' ids unique');for(const item of set)assert.equal(item.label.length,5)}
  // The UAE focus swaps the educational examples for native species while the catalogue species still lead.
  const uae=sceneItems('desert',[species('camel','Camel')],200,6,'AE').filter(i=>i.kind==='animal'||i.kind==='plant').map(i=>i.id);
  assert.deepEqual(uae,['species:camel','arabian-red-fox','scorpion','ghaf','samr']);
  assert.deepEqual(sceneItems('coast',[],200).filter(i=>i.kind==='animal'||i.kind==='plant').map(i=>i.id),['greater-flamingo','green-turtle','dugong','grey-mangrove','jellyfish'],'the default focus is the UAE');
  assert.deepEqual(sceneItems('forest',[],200,6,'XX').map(i=>i.id).slice(0,2),['roe-deer','woodpecker'],'unknown focus falls back to the world examples');
});
