...
import {uaeDestinations,uaeSources,uaeRules,enrichUaeDestination} from './uae/index';
import {applyFocus,destinationCountries,terrainAnchor} from './region';
 ...
 *  Every destination carries its ISO country; `applyFocus` archives the ones outside the product focus (lib/region.ts). */
export const seedCatalog:CatalogSnapshot=applyFocus({schemaVersion:1,revision:0,publishedAt:'2026-09-17T00:00:00.000Z',destinations:[...[...structuredClone(destinations).map((d,i)=>enrichOriginalDestination({...d,timezone:timezones[i],archived:false})),...eastDestinations,...worldDestinations].map(d=>enrichUaeDestination({...d,country:d.country??destinationCountries[d.id]})),...uaeDestinations],sources:[...structuredClone(sources),...originalAdviceSources,...eastSources,...worldSources,...uaeSources],terrainGuidance:terrains.map((terrain,index)=>({terrainId:terrain.id as TerrainId,sections:(['deeper','equipment','nature','hazard'] as const).map(key=>({id:key,title:words[key==='hazard'?'warning':key] as Localized,body:locales.map(locale=>content[locale].terrains[index][key]) as Localized,sourceIds:key==='equipment'?['essentials']:key==='hazard'?[index===3?'beach-safety':index===1?'mountain-safety':'wildlife-safety']:[]})),clothing:locales.map(locale=>content[locale].terrains[index].equipment) as Localized,transport:locales.map(locale=>content[locale].terrains[index].equipment) as Localized,group:words.groupNote as Localized})),packingRules:[...rules,...originalRules,...eastRules,...worldRules,...uaeRules]});
...
export function catalogForTerrain(catalog:CatalogSnapshot,index:number){const anchor=terrainAnchor(index);return activeDestinations(catalog).find(d=>d.id===anchor&&d.terrainId===all[index])??activeDestinations(catalog).find(d=>d.terrainId===all[index])}
