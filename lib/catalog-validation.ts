...
const destination=z.object({id:recordId,terrainId,terrainIndex:z.number().int().min(0).max(3),names:localized(160),summary:localized(2000),image,imageIsIllustrative:z.boolean(),sourceIds:refs,sections:z.array(section).min(1).max(30),species:z.array(species).max(100),lat:coordinates.shape.lat,lon:coordinates.shape.lon,timezone:z.string().refine(validTimeZone).optional(),archived:z.boolean().optional()});
